...
def save_files(path_file, link_file, name_file):
    ...
    with open("History.json", "w", encoding="windows-1251") as f:
...
def show_history():
    if not os.path.exists("History.json"):
        mb.showinfo("Информация", "Файла со ссылками пока нет")
        return
    history_window = Toplevel()
    list_t = Listbox(history_window)
    list_t.pack()

    with open("History.json", "r") as file:
        hist_link = json.load(file)
        for i in hist_link:
            list_t.insert(END, i["link_file"])
